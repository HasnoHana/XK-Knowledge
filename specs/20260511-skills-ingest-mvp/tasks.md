# Tasks: Claude Code 命令驱动的知识管理系统 MVP
...
**Purpose**: 建立 command-first MVP 的最小入口与文档骨架

- [x] T001 [Setup] 明确 `.claude/commands/xk-ingest.md` 是当前唯一 MVP 产品入口
...
- [x] T004 [Foundation] 在 `.claude/commands/xk-ingest.md` 与 direct ingest runtime 中定义执行流：输入读取、上下文收集、Prompt 注入、Agent 调用、提交控制、结果反馈
...
- [x] T007 [Foundation] 在 command/runtime 与仓库根目录下的 `CONSTITUTION.md` / `WIKI/<type>/LAWS.md` 之间建立固定读取约定，确保调用前能装配完整上下文
...
## Phase 3: User Story 1 - 通过 `/xk-ingest` 将 Raw 落库为知识页 (Priority: P1) 🎯 MVP

**Goal**: 通过 Claude Code 原生 `/xk-ingest` 命令完成单份 Raw 的知识组织、统一 mutation set 生成与原子落库，使产物成为可供后续 query/check 消费的知识页
...
## Phase 4: User Story 2 - 通过 `/xk-query` 基于索引和关联返回答案 (Priority: P2)
...
- [ ] T018 [US2] 在后续阶段实现 `/xk-query` 命令入口与问题输入协议
...
```bash
Task: "在 direct ingest runtime 中实现 runtime context 组装"
...
4. **STOP and VALIDATE**：确认 `/xk-ingest` 能在 Claude Code 直接命令流程下完成成功落库与失败回滚
...
4. 每个后续故事都必须继续遵守命令入口 + thin helper 边界
