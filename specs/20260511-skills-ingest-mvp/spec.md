...
- runtime 负责读取单份 Raw、仓库根目录下的 `CONSTITUTION.md`、候选 `WIKI/<type>/LAWS.md`、现有 `INDEX.md` / `LINK.md` 知识上下文与正式 ingest prompt，并在当前 Claude Code 风格运行面中调用 custom model。
...
4. **Given** 任一目标文件写入失败，**When** runtime 进入提交阶段，**Then** helper 必须整体回滚，不得暴露部分更新状态。
5. **Given** 用户通过 `/xk-ingest` 发起 ingest，**When** 执行完成，**Then** 用户看到的是知识摄入结果摘要，而不是底层 helper/路径拼装细节。
...
- **FR-017**: 系统 MUST 在产品蓝图中保留 `/xk-query` 与 `/xk-check` 作为后续命令入口。
...
- **FR-020**: `/xk-ingest` SHOULD 尽量避免向用户暴露 `PYTHONPATH`、绝对路径拼接或运行时内部实现细节。
...
- **SC-004**: `/xk-ingest` 的用户体验以直接命令为中心，用户不需要显式操作底层 helper 路径、环境拼接或内部运行细节。
