...
作为知识库维护者，我可以在人工筛选 Raw 文档后，通过 `xk-ingest` skill 发起一次摄入，由 AI Agent 在规则和提示词约束下完成知识重组、页面生成和索引修正，并由 helper 以原子方式提交，从而验证 skills-first 工作流可以稳定落库知识。
...
- `xk-ingest` skill 负责读取单份 Raw、仓库根目录下的 `CONSTITUTION.md`、候选 `WIKI/<type>/LAWS.md`、现有 `INDEX.md` / `LINK.md` 知识上下文与正式 ingest prompt，并在当前 Claude Code 风格运行面中调用 custom model。
- AI Agent 必须在预设提示词引导下完成 Raw chunk 划分、知识重组、类型判定、标题命名、章节组织、关系建议，以及 `wiki/index/link/log` 草稿生成。
...
4. **Given** 任一目标文件写入失败，**When** skill 进入提交阶段，**Then** helper 必须整体回滚，不得暴露部分更新状态。
5. **Given** 用户通过 skill 发起 ingest，**When** 执行完成，**Then** 用户看到的是知识摄入结果摘要，而不是底层 helper/路径拼装细节。
...
- **FR-008**: Wiki Page MUST 被定义为从 Raw 派生的知识页，而不是 Raw 的精简摘要；页面组织目标应服务于知识消费、后续 query/check 使用与稳定引用，而不是按原文顺序做缩写式复述。
- **FR-009**: 系统 MUST 为每条落库知识保存回到 Raw 的引用链；没有引用支撑的内容 MUST NOT 落库。
- **FR-010**: ingest MUST 原子地同时写入 wiki 页面并更新 `index.md`、`link.md`、`log.md`；任一写入失败时 MUST NOT 留下部分成功状态。
- **FR-011**: 系统 MUST 保留一个薄 helper 层，负责 schema 校验、目标路径物化、原子提交与失败回滚。
- **FR-012**: helper MUST NOT 用固定规则替代 AI Agent 的核心知识判断、chunk 划分、类型判定、章节组织或关系推断。
- **FR-013**: AI Agent MUST 在预设提示词引导下完成 Raw chunk 划分、知识重组、类型判定、标题命名、章节组织、关系建议与 `wiki/index/link/log` 草稿生成。
- **FR-014**: AI Agent 在生成 Wiki Page 时 MUST 以知识结构而不是原始排版为主导，对 Raw 支持的内容进行重组、归纳、抽象与归并，并允许把分散证据整理成更稳定的知识章节，但 MUST NOT 引入 Raw 之外的事实。
- **FR-015**: ingest MUST 对 AI 输出做完整性约束；若模型输出仅覆盖局部内容、缺少必要草稿、无法形成合法引用集，或虽然有引用但本质上只是 Raw 的压缩摘要而未形成知识页，系统 MUST 拒绝提交。
- **FR-016**: 系统 MUST 提供预设提示词资产，分别服务于 ingest、query、check 三条主路径，并将其视为正式交付物的一部分。
- **FR-017**: 系统 MUST 在产品蓝图中保留 `xk-query` 与 `xk-check` 作为后续 skill 入口。
- **FR-018**: `xk-query` MUST 在后续阶段基于 `index.md` 与 `link.md` 缩圈检索并返回带 Raw 引用链的答案。
- **FR-019**: `xk-check` MUST 在后续阶段对单页 wiki 做引用一致性检查，并识别“无来源陈述”和“过强结论”。
- **FR-020**: skill 入口 SHOULD 尽量避免向用户暴露 `PYTHONPATH`、绝对路径拼接或运行时内部实现细节。
- **FR-021**: MVP MUST NOT 实现完整 lint、自动矛盾网络、向量检索、多用户协作、Web UI 或复杂版本治理。
- **FR-022**: 本轮实现范围 MUST 仅覆盖 `xk-ingest`，而 `xk-query`、`xk-check` 仅保留在产品蓝图与后续规划中。
...
- **Skill Runtime Context**: skill 执行时组装的上下文包，包含 Raw、Constitution、Laws、Prompt Pack 与现有知识上下文。
...
- 仓库根目录下的 `CONSTITUTION.md` 与至少一组候选 `WIKI/<type>/LAWS.md` 可被 skill 在执行时读取。
...
- **SC-003**: 在成功的 ingest 案例中，生成出的 Wiki Page 必须能体现基于 Raw 证据的知识重组与归纳，而不是沿原文顺序压缩形成的一篇缩写摘要。
- **SC-004**: `xk-ingest` 的用户体验以 skill 为中心，用户不需要显式操作底层 helper 路径、环境拼接或内部运行细节。
- **SC-005**: 在当前 Claude Code 风格环境与 custom model 运行面下，系统能够稳定完成至少一个真实 Raw 文档的 ingest 演示闭环。
- **SC-006**: 产品规格完整保留 `xk-query` 与 `xk-check` 的后续目标与约束，但本轮 implementation scope 不扩展到这两个能力。
- **SC-007**: 在结构化输出不合法、引用非法、完整性不足、仅形成压缩摘要而未形成知识页，或任一落盘失败时，helper 能稳定拒绝提交并返回可诊断的失败结果。
