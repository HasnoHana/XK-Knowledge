...
> **日期**：2026-05-12  
> **版本**：v1.0  
> **适用读者**：分布式系统学习者、后端工程师、面试准备者

---

## 目录

1. [引言：为什么需要 Raft？](#1-引言为什么需要-raft)
2. [Raft 核心设计思想](#2-raft-核心设计思想)
3. [核心概念与术语](#3-核心概念与术语)
4. [Leader 选举机制](#4-leader-选举机制)
5. [日志复制机制](#5-日志复制机制)
6. [安全性保证](#6-安全性保证)
7. [成员变更（Membership Changes）](#7-成员变更membership-changes)
8. [日志压缩（Log Compaction）](#8-日志压缩log-compaction)
9. [Raft vs Paxos 对比](#9-raft-vs-paxos-对比)
10. [实际应用与工程实践](#10-实际应用与工程实践)
11. [常见问题与面试考点](#11-常见问题与面试考点)
12. [总结](#12-总结)
...
def request_vote(req):
    ...
    # 3. 如果 votedFor 为空或等于 candidateId，且 candidate 日志至少一样新
    if (votedFor is None or votedFor == req.candidateId) and        is_log_at_least_as_new(req.lastLogTerm, req.lastLogIndex, 
                              log[-1].term, len(log) - 1):
...
def append_entries(req):
    ...
    # 2. 重置选举超时，转为 Follower
    reset_election_timeout()
    state = FOLLOWER
    currentTerm = req.term

    # 3. 如果 log 在 prevLogIndex 处没有条目，或 term 不匹配
    if len(log) <= req.prevLogIndex or        log[req.prevLogIndex].term != req.prevLogTerm:
        return {term: currentTerm, success: False}
